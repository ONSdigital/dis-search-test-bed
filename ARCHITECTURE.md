...
A guide for anyone reading this codebase, or reviewing a merge request against it, for the first time. It explains **what the project is trying to achieve** and **how the pieces fit together**. Every section points at a real file and line (e.g. [`app/evaluate.go:215`](app/evaluate.go#L215)) so you can jump straight to the code.
...
- **`compare`**: run the evaluation and log a score line per term.
- **`export`**: run the same evaluation, then write the ranked results and their current judgements to a CSV.
...
flowchart TD
    ...
    F --> G["For each TERM: build baseline query,<br/>MultiSearch to ranked doc ids, look up answer key"]
    G --> H["Score: DCG, IDCG, NDCG<br/>scoring/scoring.go"]
    H --> L1["compare: log one line per term (ui.Info)"]
...
3. [`App.Compare`](app/app.go#L34) and [`App.Export`](app/app.go#L43) both start a throwaway Elasticsearch via [`withElasticsearch`](app/elasticsearch.go#L24) (create the index, load the documents, evaluate every term, then tear the container down); `compare` logs the scores and `export` also writes them to CSV. [`App.Import`](app/import.go#L48) does not touch Elasticsearch.
4. Per term, [`App.evaluateTerms`](app/evaluate.go#L85) builds a query, runs it, and scores the result, logging one line like:
...
- a judgement file's `query_id` maps to `terms[*].id` (and the judgement's **filename** is also the term id, so the code can fetch a term's answer key directly by id, see [`relevanceForTerm`](app/evaluate.go#L192)).
...
The available algorithm names are defined in [`algorithm/option.go:9`](algorithm/option.go#L9) (`baseline`, `unweighted`), and the parameters a caller supplies (term, index, from, size, and so on) are [`SearchParameters`](algorithm/option.go#L51). The baseline builder, which performs one term search plus count aggregations, is [`NewSearchRequestBuilderBaseline`](algorithm/request.go#L100).
...
The CLI is built with [Cobra](https://cobra.dev/), and the business logic lives in a separate [`app`](app/) package so the commands stay thin. The `cmd` package only wires Cobra; [`app.App`](app/app.go#L17) owns the operations, and [`app.New`](app/app.go#L24) builds it from the embedded stores (documents, terms, judgements). Only documents get indexed into Elasticsearch; terms and judgements are used for evaluation.
...
- **`compare`** ([`cmd/compare.go`](cmd/compare.go#L8)) calls [`App.Compare`](app/app.go#L34): start a throwaway Elasticsearch, index the documents, evaluate every term, and log one score line per term. This is the core evaluation flow described in section 2.
- **`export`** ([`cmd/export.go`](cmd/export.go#L11)) calls [`App.Export`](app/app.go#L43): the same evaluation, then write each ranked result and its current relevance judgement to a CSV via [`writeEvaluationsCSV`](app/export.go#L54). It requires an output path (`-o`/`--output`).
...
`compare` and `export` share the Elasticsearch orchestration in [`withElasticsearch`](app/elasticsearch.go#L24) and the evaluation loop in [`App.evaluateTerms`](app/evaluate.go#L85). `import` is a separate, filesystem-only path in [`app/import.go`](app/import.go).
...
**How the harness actually scores a term** ([`scoreRanking`](app/evaluate.go#L215)):

1. Take the returned document ids in rank order and map each to its grade from the answer key; a returned id that wasn't judged defaults to **0** ([`app/evaluate.go:218`](app/evaluate.go#L218)).
2. `dcg = CalculateDCG(actual)` over the full returned ranking ([`app/evaluate.go:226`](app/evaluate.go#L226)).
3. `idcg = CalculateIDCG(judged)` over all judged grades for the term ([`app/evaluate.go:227`](app/evaluate.go#L227)).
4. `ndcg = dcg / idcg`, computed inline ([`app/evaluate.go:229`](app/evaluate.go#L229)).
...
**Terminology.** You will see both "queries" and "terms" for the same thing. The concept was renamed from _query_ to _term_, but the JSON and Go still use the older field names `query_id` / `Query` (see the [`term`](app/evaluate.go#L27) and [`judgement`](app/evaluate.go#L34) structs). Read "term" and "query" as synonyms.
...
```sh
go run . compare                 # evaluate every term and log the scores
...
You'll get one line per term reporting `DCG`, `IDCG` and `NDCG` ([`app/evaluate.go:169`](app/evaluate.go#L169)).
